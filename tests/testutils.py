...
def diff_pdf_htmls(original_filename, filename):
    ...
    for fname in [original_filename, filename]:
        with open(fname) as f:
            data = f.read()
            data = re.sub(
                r'name="date" content="(.*)"', r'name="date" content="%%DATE%%"', data
            )
...
def pdftoxml(filename, output):
    ...
    p = subprocess.Popen(
        ["pdftohtml", "-stdout", "-xml", "-noframes", "-i", "-q", filename],
...
class BoletoTestCase(unittest.TestCase):
    ...
    @skipIf(sys.version_info >= (3,), "Reportlab unavailable on this version")
